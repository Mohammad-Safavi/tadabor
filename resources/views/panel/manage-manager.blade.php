                                        ...
                                        <tbody>
                                            @foreach ($users as $user)
                                                <tr>
                                                    ...
                                                    <td>{{ $user->username }}</td>
                                                    <td>
                                                        <form action="{{ Route('manage.update', $user->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('PUT')
                                                            <select name="type" onchange="this.form.submit()">
                                                                <option value="" @if ($user->type == '') {{ 'selected' }} @endif>کاربر
                                                                    عادی</option>
                                                                <option value="jcd203@03id_30wlsflasl" @if ($user->type == 'jcd203@03id_30wlsflasl') {{ 'selected' }} @endif>ادمین
                                                                </option>
                                                            </select>
                                                        </form>
                                                    </td>
